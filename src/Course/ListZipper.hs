...
hasLeft ::
  ListZipper a
  -> Bool
hasLeft (ListZipper ls _ _) = not $ isEmpty ls
...
hasRight ::
  ListZipper a
  -> Bool
hasRight (ListZipper _ _ rs) = not $ isEmpty rs
...
findLeft p (ListZipper ls x rs) =
  case break p ls of
    (ls', x' :. xs) -> IsZ $ ListZipper xs x' $ ls' ++ (x :. rs)
    _ -> IsNotZ
...
findRight p (ListZipper ls x rs) =
  case break p rs of
    (rs', x' :. xs) -> IsZ $ ListZipper ((reverse rs') ++ (x :. ls)) x' xs
    _ -> IsNotZ
...
moveLeftLoop (ListZipper _ x rs) = let (r :. rs') = reverse rs in
  ListZipper (rs' ++ (x :. Nil)) r Nil
...
moveRightLoop (ListZipper ls x _) = let (l :. ls') = reverse ls in
  ListZipper Nil l (ls' ++ (x :. Nil))
...
moveRight r = IsZ . moveRightLoop $ r
...
moveLeftN ::
  ...
  -> MaybeListZipper a
moveLeftN n lz =
  if n >= 0 then P.iterate (asMaybeZipper moveLeft) (IsZ lz) P.!! n
  else P.iterate (asMaybeZipper moveRight) (IsZ lz) P.!! (- n)
...
nth ::
  ...
  -> MaybeListZipper a
nth n lz =
  let nl = (length . lefts $ lz) in
  if n < nl then moveLeftN (nl - n) lz
  else moveRightN (n - nl) lz
...
-- prop> rights (end lz) == Nil
  ...
  case moveRightN (length . rights $ lz) lz of
    (IsZ lz') -> lz'
    _ -> lz
...
start ::
  ...
  -> ListZipper a
start lz =
  case moveLeftN (length . lefts $ lz) lz of
    (IsZ lz') -> lz'
    _ -> lz
...
instance Extend ListZipper where
  f <<= lz =
    ListZipper
    (unfoldr (\lz' -> let mlz = moveLeft lz' in
               case mlz of
                 IsZ lz'' -> Full (f lz'', lz'')
                 _ -> Empty)
     lz)
    (f lz)
    (unfoldr (\lz' -> let mlz = moveRight lz' in
               case mlz of
                 IsZ lz'' -> Full (f lz'', lz'')
                 _ -> Empty)
     lz)
...
instance Traversable ListZipper where
  traverse f (ListZipper ls x rs) =
    lift3 ListZipper (traverse f ls) (f x) (traverse f rs)
